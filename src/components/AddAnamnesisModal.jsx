...
export default function AddAnamnesisModal({ isOpen, onClose, patient, onAnamnesisAdded }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm transition-opacity">
      <div className="relative w-full max-w-2xl p-8 bg-zinc-900 border border-white/10 rounded-3xl shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
        ...
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-white mb-2">Registrar Evolução / Anamnese</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Queixa Principal *</label>
            <textarea
              value={formData.queixa_principal}
              onChange={(e) => setFormData({ ...formData, queixa_principal: e.target.value })}
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 min-h-[100px] resize-y"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Histórico Familiar</label>
            <textarea
              value={formData.historico_familiar}
              onChange={(e) => setFormData({ ...formData, historico_familiar: e.target.value })}
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 min-h-[80px] resize-y"
              placeholder="Condições hereditárias, histórico genético, etc..."
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Observações Iniciais / Exame Físico</label>
            <textarea
              value={formData.observacoes_iniciais}
              onChange={(e) => setFormData({ ...formData, observacoes_iniciais: e.target.value })}
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 min-h-[120px] resize-y"
              placeholder="Anotações gerais do profissional..."
            />
          </div>

          <div className="pt-4 flex justify-end gap-3 border-t border-white/5">
            <button type="button" onClick={onClose} className="px-6 py-2.5 text-sm font-medium text-slate-300 hover:text-white transition-colors">Cancelar</button>
