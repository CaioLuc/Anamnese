        ...
        <form onSubmit={handleSubmit} className="space-y-10 mt-6">
            ...
            <section className="bg-white/[0.02] p-6 rounded-3xl border border-white/5 shadow-lg">
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-3 gap-4 bg-zinc-950 p-4 rounded-xl border border-white/5">
                        <div>
                           <label className="block text-xs font-medium text-slate-400 mb-1">Qtd. Irmãos</label>
                    ...
                    <div>
                       <label className="block text-xs font-medium text-slate-400 mb-1">Outras Observações Familiares</label>
                       <textarea name="observacoes_familiares" value={formData.observacoes_familiares} onChange={handleChange} rows="3" className="w-full p-3 bg-zinc-950 border border-white/10 rounded-xl text-white resize-y custom-scrollbar" />
                    </div>
                </div>
            </section>
            ...
            <section className="bg-white/[0.02] p-6 rounded-3xl border border-white/5 shadow-lg">
                ...
                <div className="space-y-4">
                    <div>
                       <label className="block text-xs font-medium text-slate-300 mb-2">Qual o motivo da procura pelo atendimento psicológico?</label>
                       <textarea name="motivo_consulta" value={formData.motivo_consulta} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-indigo-500" />
                    </div>
                    <div>
                       <label className="block text-xs font-medium text-slate-300 mb-2">Histórico das dificuldades relatadas (Desde quando apresenta a queixa? Houve piora recente?)</label>
                       <textarea name="historico_queixa" value={formData.historico_queixa} onChange={handleChange} rows="4" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-indigo-500" />
                    </div>
                </div>
            </section>
            ...
            <section className="bg-white/[0.02] p-6 rounded-3xl border border-white/5 shadow-lg">
                ...
                <div className="space-y-4">
                    <div>
                       ...
                       <textarea name="dinamica_familiar" value={formData.dinamica_familiar} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-indigo-500" />
                    </div>
                    <div>
                       <label className="block text-xs font-medium text-slate-300 mb-2">Dinâmica da Área Profissional / Acadêmica</label>
                       <textarea name="dinamica_profissional" value={formData.dinamica_profissional} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-indigo-500" />
                    </div>
                    <div>
                       <label className="block text-xs font-medium text-slate-300 mb-2">Dinâmica da Vida Amorosa / Relacional</label>
                       <textarea name="dinamica_amorosa" value={formData.dinamica_amorosa} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-indigo-500" />
                    </div>
                </div>
            </section>
            ...
            <section className="bg-white/[0.02] p-6 rounded-3xl border border-white/5 shadow-lg relative overflow-hidden">
                ...
                <div className="space-y-4 relative z-10">
                    <div>
                       <label className="block text-xs font-medium text-slate-300 mb-2">Sintomas Apresentados Atualmente</label>
                       <textarea name="sintomas_apresentados" value={formData.sintomas_apresentados} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-red-500/50" />
                    </div>
                    <div>
                       <label className="block text-xs font-medium text-slate-300 mb-2">Quais eventos/fatores costumam precipitar ou agravar suas crises?</label>
                       <textarea name="fatores_agravantes" value={formData.fatores_agravantes} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-red-500/50" />
                    </div>
                    <div>
                       <label className="block text-xs font-medium text-slate-300 mb-2">Histórico de Transtornos Psiquiátricos (Anteriores e Familiares)</label>
                       <textarea name="transtornos_anteriores" value={formData.transtornos_anteriores} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-red-500/50" />
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                           <label className="block text-xs font-medium text-slate-300 mb-2">Doenças Importantes (Antecedentes Médicos)</label>
                           <textarea name="doencas_importantes" value={formData.doencas_importantes} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-red-500/50" />
                        </div>
                        <div>
                           <label className="block text-xs font-medium text-slate-300 mb-2">Medicações em Uso Atualmente</label>
                           <textarea name="medicamentos" value={formData.medicamentos} onChange={handleChange} rows="3" className="w-full p-4 bg-zinc-950/80 border border-white/10 rounded-xl text-white resize-y custom-scrollbar focus:ring-1 focus:ring-red-500/50" />
                        </div>
                    </div>
