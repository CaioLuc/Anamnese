      ...
      <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/5 shadow-2xl rounded-3xl overflow-hidden relative">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-64 p-8">
            ...
            <span className="text-slate-400 font-medium">Buscando dados no Firebase...</span>
          </div>
        ) : patients.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-16 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-300">Nenhum paciente na base</h3>
            <p className="mt-2 text-slate-500 max-w-sm">Você ainda não possui pacientes cadastrados. Clique no botão acima para registrar seu primeiro atendimento.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-zinc-800/30 border-b border-white/5 text-xs uppercase tracking-wider text-slate-400 font-semibold">
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-10 w-10 mr-4">
                            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-indigo-500/20 to-cyan-500/20 border border-indigo-500/30 flex items-center justify-center text-sm font-bold text-indigo-300">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-500 mt-0.5">ID: {patient.id.substring(0, 8)}...</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap hidden sm:table-cell">
                        <div className="text-sm text-slate-300">{age > 0 ? `${age} anos` : '-'}</div>
                        <div className="text-xs text-slate-500">{new Date(patient.data_nascimento).toLocaleDateString()}</div>
                      </td>
