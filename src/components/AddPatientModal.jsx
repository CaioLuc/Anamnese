...
export default function AddPatientModal({ isOpen, onClose, onPatientAdded }) {
  const [formData, setFormData] = useState({ nome: '', data_nascimento: '', telefone: '', cpf: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ nome: '', data_nascimento: '', telefone: '', cpf: '' });
      onClose();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-zinc-950/80 backdrop-blur-sm">
      <div className="relative w-full max-w-md p-8 bg-zinc-900 border border-white/10 rounded-3xl shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar pointer-events-auto">
        ...
        <h2 className="text-2xl font-bold text-white mb-6">Novo Paciente</h2>
